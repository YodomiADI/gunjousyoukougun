...
func save_game():
	...
	print("セーブしました: ", current_chapter_id, " ", current_line_index)
...
func load_game():
	...
	if error == OK:
		var data = json.data
		# 保存したデータを変数に戻す
		current_chapter_id = data.get("chapter_id", "prologue") # 見つからなければprologue
		current_line_index = data.get("line_index", 0)
		print("ロードしました: ", current_chapter_id, " ", current_line_index)
		return true # ロード成功
	else:
		print("ロードに失敗しました。")
		return false # ロード失敗
